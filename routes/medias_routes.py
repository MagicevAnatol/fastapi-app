...
async def upload_media(
    ...
    media_id = await db_handlers.save_media(db, filename=file.filename, file_data=file_data)
...
async def get_media(media_id: int, db: AsyncSession = Depends(get_db)):
    ...
    if media:
        return StreamingResponse(BytesIO(media.file_data), media_type="application/octet-stream", headers={"Content-Disposition": f"attachment; filename={media.filename}"})
    else:
