...
async def test_create_tweet_with_media_ids(async_client):
    ...
    response = await async_client.post("/api/tweets/", headers={"api-key": "test"}, json=tweet_request)
    assert response.status_code == 200
    assert response.json()["result"] is True
...
async def test_delete_tweet(created_tweet_id, async_client):
    ...
    response = await async_client.delete(f"/api/tweets/{tweet_id}", headers={"api-key": "test"})
    assert response.status_code == 200
...
async def test_delete_not_owned_tweet(async_client):
    ...
    assert response.json()["result"] == "false"
    assert response.json()["error_type"] == "HTTPException"
    assert response.json()["error_message"] == "You are not allowed to delete this tweet"


@pytest.mark.asyncio
async def test_unlike_tweet(async_client):
    response = await async_client.delete("/api/tweets/2/likes", headers={"api-key": "test"})
    assert response.status_code == 200
    assert response.json()["result"] is True
...
async def test_like_tweet(async_client):
    response = await async_client.post("/api/tweets/2/likes", headers={"api-key": "test"})
    assert response.status_code == 200
    assert response.json()["result"] is True